...
/* simple ringmic test require:
* 1. it can used to replace echo_ringmic_test for mic test
* 2. it can used to replace echo_audio_record_test in auto mode
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static int check_data_right(int signal, int input[], int input_length)
{
	...
	while(i < input_length)
	{
        ...
        if(signal ==5)
	        printf("check agreemnt, input[i] is 0x%x , data_fre is 0x%x, sample is %d  \n",input[i], data_fre, data_fre);
...
int main()
{
	...
	system("amixer set Master Playback 90%");
	usleep(800000);
	...
	*start++ = ';';
    ...
    log_info("after record: ispass = %d", ispass);
	if (!ispass) {
		strcpy(result, RESULT_FAIL);
