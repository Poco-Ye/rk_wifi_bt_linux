...
static int isRecoveryMode()
{
    ...
    if(strncmp(msg.command, "boot-recovery", strlen("boot-recovery")) == 0 &&
        strncmp(msg.recovery, "recovery", strlen("recovery")) == 0)
        return 1;
    else
        return 0;
}
...
void fastoryDataReset(){
    struct bootloader_message msg;
    memset(&msg, 0, sizeof(msg));
    strncpy(msg.command, "boot-recovery", sizeof(msg.command));
    strncpy(msg.recovery, "recovery\n--wipe_all", sizeof(msg.recovery));
    strncat(msg.recovery, "\n", sizeof(msg.recovery));
    strncpy(msg.systemFlag, "false", sizeof(msg.systemFlag));
...
static int get_verdor_parameter(int cmd, char *param_buf, int len)
{
    ...
    log_info("get_verdor_parameter: enter...\n");
    sys_fd = open(VERDOR_DEVICE, O_RDWR, 0);
    if (sys_fd < 0) {
        log_err("get_verdor_parameter: error with sys_fd < 0\n");
        return ret;
    }
    ...
    if (ioctl(sys_fd, VENDOR_READ_IO, &req)) {
        log_err("get_verdor_parameter: cmd = [%d] fail\n", cmd);
        log_info("\t VENDOR_SN_ID        = 1\n");
        log_info("\t VENDOR_WIFI_MAC_ID  = 2\n");
        log_info("\t VENDOR_LAN_MAC_ID   = 3\n");
        log_info("\t VENDOR_BLUETOOTH_ID = 4\n");
        close(sys_fd);
        return -1;
    }
    /* rknand_print_hex_data("vendor read:", (uint32_t*)&req, req.len + 8); **/
    if (req.len > len) {
        log_info("get_verdor_parameter:%d force to %d\n", req.len, len);
        req.len = len;
        ret = len;
    } else {
        log_info("get_verdor_parameter req.len:%d  len = %d\n", req.len, len);
    }
    ...
    close(sys_fd);
    log_info("get_verdor_parameter: success with cmd = [%d], paramter = [%s]\n",cmd, param_buf);
