...
#define KEY_FILE "/data/keytest.json"

static int  KEY_VALID_NUM = 0;    //3308 evb v11
...
static int init_keyset()
{
    ...
    if(NULL != (fp = fopen("/tmp/factory/keytest.json", "r"))) {
        while (NULL != fgets(buf, sizeof(buf), fp)) {
            buf[strlen(buf)-1]='\0';
            strcat(str,buf);
        }
    }else if(NULL != (fp = fopen("/etc/keytest.json", "r"))) {
        while (NULL != fgets(buf, sizeof(buf), fp)) {
            buf[strlen(buf)-1]='\0';
            strcat(str,buf);
        }
    }else{
        printf("parase keytest.json file error! \n");
        printf("no found  keytest.json file in /etc/ or /tmp/factory/ \n");
        return -1;
    }
