...
#define LED_BRIGHTNESS_FILE "/sys/class/leds/%sled/brightness"
#define LED_TRIGGER_FILE "/sys/class/leds/%sled/trigger"
#define LED_SHOT_FILE "/sys/class/leds/%sled/shot"
#define LED_DELAY_ON_FILE "/sys/class/%sleds/led/delay_on"
#define LED_DELAY_OFF_FILE "/sys/class/%sleds/led/delay_off"
    ...
    while(color_num < 4)
    {
        ...
        for(led_num=1;led_num<=LED_NUM;led_num++)
        {
            ...
            case LED_RED :
                if(color_num % 3 == 1)
                {
                    sprintf(buf,LED_BRIGHTNESS_FILE,"red");
            ...
            case LED_GREEN :
                if(color_num % 3 == 2)
                {
                    sprintf(buf,LED_BRIGHTNESS_FILE,"green");
                    if(-1==write_int(buf,LED_ON))
                        goto fail;
                }
                else
            ...
            case LED_BLUE :
                if(color_num % 3 == 0)
                {
                    sprintf(buf,LED_BRIGHTNESS_FILE,"blue");
                    if(-1==write_int(buf,LED_ON))
                        goto fail;
                }
                else
