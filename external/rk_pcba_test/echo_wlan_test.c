...
#define GET_WLAN_TEST_EXIST  "ps -ef | grep 'echo_wlan_test' | grep -v 'grep' | awk 'NR>1 {print $1}'"
...
int main(int argc, char *argv[])
{
    ...
    sprintf(tmp,"echo '%s<%s>,<%s>,<%d>' > /tmp/echo_wlan_test_result", TESTITEM_SEND_HEAD, buf, result, err_code);
    system(tmp);
    system("sync");
}
